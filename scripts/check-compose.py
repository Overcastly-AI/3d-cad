...
CI never runs ``docker compose up`` (and this container can't — registry
blocked), so `docker compose config -q` alone would let these regressions
back in silently. This guard renders the compose config (no daemon or image
pulls needed) and asserts the invariants the 2026-07-24 audit found violated:
