...
# The HTTP + modeling helpers are shared with scripts/backup-verify.py (the
# backup/restore drill drives the SAME feature tree), so they live in one
# module. Hyphenated script names are not importable, hence the path insert.
...
def run(base_url: str, host: str) -> None:
    ...
    document = assert_glb_has_geometry(mesh.body)
