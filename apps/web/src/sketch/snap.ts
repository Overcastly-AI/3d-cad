...
/**
 * Every kind the cursor can report. The two `axis-*` kinds are not entity
 * snaps — they are the Shift axis lock, reported through the same channel
 * because they answer the same question the mark exists to answer: what will
 * this click actually give me?
 */
export type SnapKind = EntitySnapKind | "axis-h" | "axis-v";
 ...
 * Endpoints sit alone at the top because they are the most-wanted snap and
 * they stack (two lines meeting at a corner); midpoint and centre share a rank
 * because between a line's midpoint and a nearby circle's centre the NEARER one
 * is always what was meant; derived-from-a-pair snaps rank below derived-from-
 * one; and the two that need an anchor point (tangent, perpendicular) rank
 * last, since they only exist mid-placement.
 */
const RANK: Record<EntitySnapKind, number> = {
  endpoint: 0,
  midpoint: 1,
  center: 1,
  intersection: 2,
...
export function snapCandidates(
  ...
  const offer = (kind: EntitySnapKind, point: Point2D, ids: string[]): void => {
    const d = dist(at, point);
    // Rule 1: the mark never jumps away from the cursor.
    if (d <= toleranceMm) found.push({ kind, at: point, entities: ids, d });
  };
  ...
  return found.map(({ kind, at: point, entities: ids }) => ({
    kind,
    at: point,
    entities: ids,
  }));
