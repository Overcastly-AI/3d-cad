...
import {
  placeProposal,
  type ProposalObstruction,
  type ProposalPlacement,
} from "./sketchProposal";

/**
 * Is this gesture aimed at the SCENE, or at a control that floats over it?
 *
 * `data-viewport-chrome` is the element's own declaration that it docks over
 * the viewport — `fitFraming.measureChrome` already reads it so a fit never
 * tucks the model under a panel, and the chip's keep-out now reads it too.
 * Asking the same question here means a new corner widget behaves correctly
 * for all three without being named anywhere.
 */
function onViewportChrome(target: EventTarget | null): boolean {
  return (
    target instanceof Element &&
    target.closest("[data-viewport-chrome]") !== null
  );
}
...
export function ProposalNote({
  ...
  /**
   * The chrome the chip must not cover, read LIVE at placement time — the same
   * rects, from the same declaration, that a camera fit refuses to tuck the
   * model under. Live rather than a constant because that is the difference
   * between a keep-out that stays true and one that goes stale the next time a
   * panel collapses or a widget moves: a hardcoded cube rect would have been
   * wrong the day the cube changed size.
   */
  const chromeRects = useCallback((): ProposalObstruction[] => {
    const container = containerOf();
    return container === null ? [] : measureChrome(container).obstructions;
  }, [containerOf]);
  ...
  //
  // A gesture on the viewport's CHROME is not one of those gestures, and the
  // asymmetry with the pointer note below is deliberate rather than an
  // oversight. THE COST OF A WRONG DISMISSAL IS WHAT DIFFERS: the pointer note
  // is regenerated by resting on the face again, so withdrawing it on a camera
  // steer is free and correct — its anchor is a fixed screen point that the
  // steer invalidates. This offer is ONE-SHOT and `seen` is recorded at write
  // time, so a click that lands on the reference cube would spend a sketch's
  // extrude offer FOR THE SESSION, with no way to ask for it back. Reaching for
  // the cube to look at the profile you just solved is preparation for taking
  // the offer, not an abandonment of it; and this note's anchor re-projects per
  // frame (`SolveProposalAnchor`), so it stays truthful across the view change
  // instead of pointing at where the profile used to be. The chip's own
  // exemption on the next line was always this rule applied to one element.
  useEffect(() => {
    const container = containerOf();
    if (container === null) return;
    const dismiss = (event: Event) => {
      if (chipRef.current?.contains(event.target as Node) === true) return;
      if (onViewportChrome(event.target)) return;
      withdrawOffer();
    };
  ...
  }, [note, extrudeEnabled, profile, anchor, chromeRects]);
