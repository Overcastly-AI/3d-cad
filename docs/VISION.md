...
This table no longer starts near-all-❌: three pillars are ✅ (Sketching, Part
modeling, Price/freedom), four hold at ➖ — genuinely usable daily-driver
capability, short of incumbent parity on named residuals (Assemblies,
Interop, Drawings, Sheet metal) — and four are still ❌, honest Phase-5+
surfaces not yet built (Performance benchmarking, Collaboration, Extensibility,
Agent access). The loop's job is to keep flipping rows and never let this
table go stale.

Last re-scored 2026-07-19 (vision-steward), tenth pass this cycle, against
