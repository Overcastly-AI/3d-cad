...
| Redundant-vs-conflicting diagnosis | — (not verified this pass) | — | ✅ | **Shipped, classified, on the ADD-A-CONSTRAINT path** (`b28dffc`→`c527063`→`c4527f6` backend, `4e6e429` frontend, 2026-07-15): typed `SketchConstraintDiagnosis` — classification (redundant vs. conflicting), removable, named offending ids, suggested fix. **Downgraded ✅→🚧 2026-08-21 (vision-steward)** on `docs/AUDIT-PRODUCT.md` R-5's claim that a dimension-VALUE edit bypasses this diagnosis entirely. **Restored 🚧→✅ 2026-08-22 (this pass) — the R-5 mechanism was retracted, by two independent sources, before a fix was even needed for THIS row.** `docs/AUDIT-ENGINEERING.md`'s "Pass 8" traced `solve()` directly and found it stateless — a value edit and a new constraint are the identical code path, so there is no separate route to bypass — and measured the ORIGINAL pre-fix bytes returning `status=conflicting` with 12 offending indices for a pure value edit on an axis-aligned profile. `docs/BACKLOG.md`'s SOLVE-1 fix (`7183955`, closed) confirms this: the real defect was solver non-idempotence (an under-constrained solve was not returning to the same answer on retyping the original value), not a missing diagnosis route — and R-5c specifically was not a conflict at all, since the profile had no H/V constraints authored (see the "Automatic constraint inference" row below, residual (3)/SNAP-5) and the six side lengths in question close geometrically without contradiction. Diagnosis on BOTH paths — new-constraint authoring and existing-value editing — is the same mechanism and both are exercised; see VISION.md's Sketching row (flipped back ➖→✅ this pass) for the full chain. |
...
named needle-movers, in that order. **Note added 2026-08-21, not a re-derivation of the coarse table above (still stale, still deferring to the parity doc):** the flat-pattern DXF DELIVERABLE — this row's own claimed strength — had four real fidelity defects an audit found and all four are now closed: wrong scale (inherited the drawing sheet's view scale instead of 1:1), no one-click profile-only export (only available wrapped in a full drawing sheet), bend-table text sharing a layer with cut geometry, and a codepage-vs-encoding mismatch producing mojibake. Full evidence in VISION.md's Sheet metal row Notes, 2026-08-21 correction. None of the four are independently reviewed/QA'd yet. **Correction 2026-08-22 (vision-steward) — VISION.md's Sheet metal row flipped ➖→❌ this pass; this coarse index is updated to match, per this section's own stated rule.** A second, deliberately-different audit job (fabrication handoff: bracket → flat pattern → assembly → print) found the F-1/F-2/F-3 fixes above did not generalize to a bracket with real through-holes: **DXF-4** (P0, open) — the flat pattern of a 4-hole bracket ships **zero circles**, the unfold itself drops every through-feature, not just the writer; **DXF-5** (P0, open) — `$INSUNITS` declares metres on a millimetre file (a 1000x error), contradicting this section's own prior "F-1 closed" framing that the scale problem was solved — the scale of the GEOMETRY was fixed, the UNIT DECLARATION was a separate, still-open bug; **EDGEFLANGE-1** (P0, open) — an edge flange can be built off the sheet's own thickness edge (physically impossible) and reports success with no warning. Verified against `docs/BACKLOG.md`'s live tickets, corroborated against Fusion's flat-pattern documentation this pass (`WebSearch`, `help.autodesk.com/view/fusion360/ENU/?contextId=SM-FLAT-PATTERN`: holes are ordinary flat-pattern content by default). Full reasoning for ❌ (not a domain rollback — base/edge flange, closed hem, corner relief remain shipped and correct) is in VISION.md's Sheet metal row Notes.
...
- **2026-08-22 (1-day gap — reconciliation-dominant, one targeted WebSearch pass, triggered by SOLVE-1 shipping and by BACKLOG groom passes 9/10 filing six more P0s from a second, deliberately-different audit job).** Re-derived, not inherited: read `services/geometry/src/geometry/sketch/planegcs_solver.py` directly at the cited call sites and re-ran its regression suite (24 tests, green) before crediting SOLVE-1/SETTLE-2/SETTLE-3's claims. **Reconciliation: three scorecard-adjacent corrections.** (1) The "Redundant-vs-conflicting diagnosis" row, downgraded ✅→🚧 by the pass below on R-5's "value edits bypass diagnosis" claim, is restored 🚧→✅ — `docs/AUDIT-ENGINEERING.md`'s "Pass 8" traced `solve()` and found it stateless (no separate route to bypass), and SOLVE-1's own measurement shows the ORIGINAL pre-fix bytes already returning `status=conflicting` on a pure value edit; the real defect was solver non-idempotence, now closed. (2) "Automatic constraint inference"'s residual (3) (no auto-H/V on a drawn line) is promoted from a noted-but-unfiled P2/S candidate to a filed, dispatched ticket (**SNAP-5**, P1/S) — SOLVE-1's own reproduction is independent severity evidence (a consistent axis-aligned profile solved at DOF 6, not the DOF 2 an H/V-inferring incumbent would carry). (3) Sheet metal's coarse index and the Assemblies row both gain corrections matching two VISION.md score flips this pass (below). **Fresh discovery, one targeted `WebSearch` pass** against `help.autodesk.com/view/fusion360/ENU/?contextId=SM-FLAT-PATTERN` (Fusion's flat-pattern documentation, read to corroborate — not originate — the severity of `docs/BACKLOG.md`'s **DXF-4**: Fusion's own docs treat a hole/cutout as ordinary flat-pattern content by default, special-casing only the unusual case of adding a NEW hole from inside the flattened view, confirming "holes appear in the flat pattern" is baseline fabricator expectation, not an advanced feature) and a general Plasticity 2026.1 release-notes search (`doc.plasticity.xyz/release-notes/whats-new-2026.1`, corroborating the already-tracked Slot command and surfacing one genuinely new item: **PolySplines**, a Studio-only third-party command converting mesh objects into editable G2-continuous NURBS surfaces — reverse-engineering, not a modeling-workflow gap on the same axis as this file's other rows, and Loft has no scanned-mesh-to-parametric pipeline to compare it against; not filed, noted for a future pass if reverse-engineering ever becomes a stated priority). **VISION.md score changes this pass, both on `docs/BACKLOG.md`'s live tickets, not on fresh WebSearch discovery:** **Sketching & constraints ➖→✅** (SOLVE-1 closed and twice independently re-hardened, SETTLE-2/SETTLE-3 — full chain in VISION.md's row); **Assemblies & mates ➖→❌** and **Sheet metal ➖→❌** (both acting on the vision-steward's own prior-cycle recommendation that had sat unactioned — MATE-1 and DXF-4 respectively, re-derived fresh against BACKLOG rather than rubber-stamped). **The conclusion, restated because the situation has gotten more acute, not less, since the pass below already said this: defect repair outranks every fresh competitive candidate in this file, without qualification, for at least one more cycle.** The second fabrication-handoff audit job that produced DXF-4/DXF-5/EDGEFLANGE-1/MATE-1/NAME-2 found P0s in a part of the product (sheet-metal-to-shop-floor, assembly mate-picking) the FIRST audit job never exercised — the same "a curated golden passes, a different ordinary session finds a wall" pattern this file's own prior entries name, recurring a third time on a third capability area. No new capability candidate found this pass outranks finishing that cluster.
